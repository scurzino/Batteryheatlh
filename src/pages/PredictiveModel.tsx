      ...
      reader.onload = (event) => {
        ...
        if (text) {
          ...
          if (missingHeaders.length > 0) {
            setError(`Invalid file format. Missing these columns: ${missingHeaders.join(', ')}`);
            setFile(null);
          } else {
            setError(null);
  ...
  const handleInference = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(errData.error || 'Error during prediction');
      }
      ...
      <div className="glass-panel ghost-border rounded-3xl p-8 mt-8">
        ...
        {predictionResult ? (
          <div className="p-8 text-center bg-primary-container/20 rounded-xl border border-primary/20 flex flex-col items-center justify-center">
            ...
            <h3 className="text-sm font-medium text-secondary mb-1">Predicted Capacity</h3>
            <div className="text-5xl font-bold font-headline text-primary">
              {predictionResult.predicted_soh?.toFixed(2) || '--'} Ah
            </div>
